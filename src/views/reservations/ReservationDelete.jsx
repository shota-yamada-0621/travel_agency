...
const ReservationDelete = () => {
  ...
  const handleDelete = () => {
    if (isLocal) {
      const local = JSON.parse(localStorage.getItem(LOCAL_KEY) || "[]");
      const filtered = local.filter((r) => r.id !== Number(id));
      localStorage.setItem(LOCAL_KEY, JSON.stringify(filtered));
      showSnackbar("予約を削除しました", "is-danger");
    }
    ...
    <div className="modal is-active">
      ...
      <div className="modal-card" style={{ borderRadius: "1.5rem", maxWidth: 420 }}>
        ...
        <section className="modal-card-body has-text-centered">
          <p className="mb-4">本当に「<strong>{reservation.name}</strong>」の予約を削除しますか？</p>
